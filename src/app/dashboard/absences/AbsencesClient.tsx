...
interface Absence {
  ...
  justified: boolean
  reason: string | null
  students: { full_name: string; parent_phone: string | null } | null
...
export default function AbsencesClient({ absences, classes, schoolName, onRefresh }: Props) {
  ...
  const currentSchoolName = schoolName || getCurrentUser()?.school_name || ''
  ...
  async function handleSubmit() {
    if (!form.student_id || !form.class_id) {
      setError('Veuillez selectionner une classe et un eleve')
      return
    }
    setLoading(true)
    setError('')
    ...
    setSuccess('Absence enregistree avec succes')
    setForm({
      student_id: '',
      class_id: '',
      absence_date: new Date().toISOString().split('T')[0],
  ...
  const filtered = absences.filter((a) => {
    const matchClass = filterClass === 'tous' || a.classes?.name === filterClass
    const matchStatus =
      filterStatus === 'tous' ||
      (filterStatus === 'justifiee' && a.justified) ||
      (filterStatus === 'non_justifiee' && !a.justified)
                ...
                {filtered.length === 0 ? (
                  ...
                  filtered.map((absence, index) => (
                    <tr key={absence.id} style={{ borderBottom: index < filtered.length - 1 ? '1px solid #F1F5F9' : 'none' }}>
                      ...
                      <td style={{ padding: '11px 16px', fontSize: '13px', color: '#475569' }}>
                        {new Date(absence.absence_date).toLocaleDateString('fr-MA')}
                      </td>
                      <td style={{ padding: '11px 16px' }}>
                      ...
                      <td style={{ padding: '11px 16px' }}>
                        <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                          ...
                          {absence.students?.parent_phone && (
                            <WhatsAppButton
                              phone={absence.students.parent_phone}
                              message={MESSAGES.absence(
                                absence.students.full_name,
                                new Date(absence.absence_date).toLocaleDateString('fr-MA', { weekday: 'long', day: 'numeric', month: 'long' }),
                                currentSchoolName
                              )}
                              size="sm"
                              variant="outline"
          ...
          <div style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: '12px', padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '14px' }}>
            ...
            <div>
              ...
              <select
                value={form.class_id}
                onChange={(e) => {
                  setForm((prev) => ({ ...prev, class_id: e.target.value, student_id: '' }))
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setForm((prev) => ({ ...prev, absence_date: e.target.value }))}
                style={inputStyle}
              />
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <input
                ...
                onChange={(e) => setForm((prev) => ({ ...prev, justified: e.target.checked }))}
                style={{ width: '16px', height: '16px', cursor: 'pointer' }}
              />
              <label htmlFor="justified" style={{ fontSize: '13px', color: '#1E293B', cursor: 'pointer' }}>
                Absence justifiee
              </label>
            </div>
            ...
            {success && (
              <div style={{ background: '#DCFCE7', border: '1px solid #86EFAC', color: '#166534', padding: '10px 14px', borderRadius: '8px', fontSize: '13px' }}>
                {success}
              </div>
            )}
