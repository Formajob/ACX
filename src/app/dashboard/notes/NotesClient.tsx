...
interface Grade { id: string; value: number; term: number; student_id: string; subject_id: string; class_id: string; students: { full_name: string } | null }
...
export default function NotesClient({ classes, subjects, grades }: Props) {
  ...
  async function handleSave() {
    if (!selectedClass || !selectedSubject) {
      setError('Selectionnez une classe et une matiere')
      return
    }
    setLoading(true)
    setError('')
    ...
    if (upserts.length === 0) {
      setError('Aucune note a enregistrer')
      setLoading(false)
      return
    }
    ...
    setSuccess(upserts.length + ' notes enregistrees avec succes')
    setLoading(false)
    router.refresh()
  }
  ...
  function getMoyenne(studentId: string) {
    ...
    const totalCoeff = studentGrades.reduce((sum, g) => {
      const subj = subjects.find(s => s.id === g.subject_id)
    ...
    <div>
      ...
      <div style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: '12px', padding: '1.25rem', marginBottom: '1.25rem' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px' }}>
          <div>
            <label style={{ fontSize: '13px', fontWeight: 500, color: '#1E293B', display: 'block', marginBottom: '5px' }}>
              Classe
            </label>
            <select
              value={selectedClass}
        ...
        <div style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: '12px', overflow: 'hidden' }}>
          <div style={{ padding: '1rem 1.25rem', borderBottom: '1px solid #E2E8F0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <button
              onClick={handleSave}
              disabled={loading}
              style={{ padding: '7px 16px', border: 'none', borderRadius: '8px', background: loading ? '#94A3B8' : '#2563EB', color: '#fff', fontSize: '13px', fontWeight: 500, fontFamily: 'DM Sans, sans-serif', cursor: loading ? 'not-allowed' : 'pointer' }}
            >
