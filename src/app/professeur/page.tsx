...
export default function ProfesseurPage() {
  ...
  const [absencesNonJust, setAbsencesNonJust] = useState(0)
  ...
  async function loadData(userId: string) {
    ...
    const [
      ...
      supabase.from('timetable_slots').select('*, subjects(name), classes(name)').eq('teacher_id', userId).order('day_of_week').limit(6),
    ])

    setTotalEleves(eleves ?? 0)
    setAbsencesNonJust(absences ?? 0)
    setRecentAbsences(absRecentes ?? [])
    setMySlots(slots ?? [])
    setLoading(false)
  }
  ...
  return (
    <div style={{ fontFamily: 'DM Sans, sans-serif' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px', marginBottom: '1.5rem' }}>
        {[
          { label: 'Mes classes',        value: mesClasses.length, color: '#2563EB', bg: '#EFF6FF' },
          { label: 'Mes eleves',         value: totalEleves,       color: '#10B981', bg: '#ECFDF5' },
          { label: 'Absences injustif.', value: absencesNonJust,   color: '#F59E0B', bg: '#FFFBEB' },
          { label: 'Creneaux / sem',     value: mySlots.length,    color: '#EF4444', bg: '#FEF2F2' },
        ].map(s => (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
        <div style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: '12px', padding: '1.25rem' }}>
          <div style={{ fontSize: '13px', fontWeight: 500, color: '#64748B', marginBottom: '1rem' }}>Mon planning</div>
          {mySlots.length > 0 ? mySlots.map((slot: any, index: number) => (
            ...
            <div style={{ textAlign: 'center', padding: '2rem', color: '#94A3B8', fontSize: '13px' }}>Aucun creneau assigne</div>
          )}
        </div>

        <div style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: '12px', padding: '1.25rem' }}>
          <div style={{ fontSize: '13px', fontWeight: 500, color: '#64748B', marginBottom: '1rem' }}>Dernieres absences</div>
          {recentAbsences.length > 0 ? recentAbsences.map((absence: any, index: number) => (
            <div key={absence.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 0', borderBottom: index < recentAbsences.length - 1 ? '1px solid #F1F5F9' : 'none', fontSize: '13px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: '11px', color: '#94A3B8' }}>{absence.classes?.name}</div>
                </div>
              </div>
            ...
            <div style={{ textAlign: 'center', padding: '2rem', color: '#94A3B8', fontSize: '13px' }}>Aucune absence recente</div>
          )}
        </div>
      </div>
